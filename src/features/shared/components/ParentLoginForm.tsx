...
export function ParentLoginForm({ parents, onSuccess, onError }: ParentLoginFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const found = parents.find(p => p.phone.trim().replace(/\s+/g, '') === cleanPhone);
    if (found) {
      onError('');
      onSuccess(found.phone);
    } else {
      onError('Aucun parent trouvé avec ce numéro. Essayez un des comptes de démonstration ci-dessous.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      <div className="space-y-1">
        <label className="block text-slate-500 font-semibold">Numéro de téléphone du Parent</label>
        <div className="relative">
          <Phone className="absolute left-3 top-3 w-4 h-4 text-slate-400" />
          <input
            id="parent-popup-phone" type="text" placeholder="+221 77 123 45 67" value={phone}
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          ...
          <input
            id="parent-popup-password" type={showPassword ? 'text' : 'password'} value={password}
      ...
      <button
        type="submit" id="parent-popup-submit"
      ...
      <div className="bg-slate-50 rounded-2xl p-3 border border-slate-200 space-y-2 text-slate-500 mt-2">
        <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider block">Parents de démo</span>
        <div className="space-y-1">
          {parents.map(p => (
            <button
